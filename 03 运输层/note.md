...
* 3.1.1 运输层和网络层的关系
 	...
 	* 运输层协议仍然能够提供网络层协议不支持的服务， 比如可靠性保证（无差错，顺序交付，无冗余）安全性保证（加密）
...
* 3.2 多路复用与多路分解
	...
	* **无连接的`多路复用`与`多路分解`**
		* **UDP** 套接字使用二元组 `(目的 IP、目的端口)` 来标识，若两个报文段 `源端口` 和 `源 IP` 不一样，但 `目的 IP` 和 `目的端口` 一样，会被定位到目的主机上相同的套接字。
	* **面向连接的 `多路复用` 与 `多路分解`**
		* **TCP** 套接字使用 `(目的 IP、目的端口、源 IP、源端口)` 来标识，若两个报文段 `源IP 或者源端口` 不同，但 `目的 IP 或者目的端口` 相同，会被定位到不同的套接字。
