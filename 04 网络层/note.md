  ...
  * 4.4 **网际协议：因特网中的转发和编址**
	...
	* **4.6 因特网中的路由选择**
		...
		* **4.6.2 因特网中自洽系统内部的路由选择 : OSPF**
			...
			* **OSPF** 的核心是一个使用洪泛链路状态信息的链路状态协议和一个 Djkstra 算法。每台路由器都构建了一幅整个 AS 的完整拓扑图，于是它可以自己运行 Djkstra 算法来确定以自己为根结点到所有子网的最短路径树。
			
			* **OSPF** 的路由器运行 Djkstra 算法所依据的链路费用是网络管理员来配置的，他可以将所有链路费用设为 1，从而找出最少跳数。也可以设置为链路容量反比的函数从而鼓励使用高带宽链路。
